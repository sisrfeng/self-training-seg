...
def parse_args():
    ...
    # basic settings
    arG('--data-root'  , type=str   , required=True)
    arG('--dataset'    , type=str   , choices=['pascal', 'cityscapes'], default='pascal')
    arG('--batch-size' , type=int   , default=16)
    arG('--lr'         , type=float , default=None)
    arG('--epochs'     , type=int   , default=None)
    arG('--crop-size'  , type=int   , default=None)
    arG('--backbone'   , type=str   , choices=['resnet50', 'resnet101'], default='resnet50')
    arG('--model'      , type=str   , choices=['deeplabv3plus', 'pspnet', 'deeplabv2'],
                        default='deeplabv3plus')

    # semi-supervised settings
    arG('--labeled-id-path'   , type=str , required=True)
    arG('--unlabeled-id-path' , type=str , required=True)
    arG('--pseudo-mask-path'  , type=str , required=True)

    arG('--save-path', type=str, required=True)

    # arguments for ST++
    arG('--reliable-id-path', type=str)
    arG('--plus', dest='plus', default=False, action='store_true',
...
def main(args):
    ...
    valset = SemiDataset(args.dataset,
                         args.data_root,
                         'val',
                         None,
                        )
    ...
    print('\n================> Total stage 1/%i: '
          'Supervised training on labeled images (Supervised Only)' % (6 if args.plus else 3))

    global MODE
    MODE = 'train'

    trainset = SemiDataset(args.dataset,
                           args.data_root,
                           MODE,
                           args.crop_size,
    ...
    best_model, checkpoints = train(model,
                                    trainloader,
                                    valloader,
                                    criterion,
                                    optimizer,
